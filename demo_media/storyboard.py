...
Now the demo intentionally changes the replenishment flow choices instead of staying on one example.
...
This rendered demo is generated by first producing the narration MP3 and then recording the Streamlit flow into an MP4 video, with the visual storyboard automatically stretched to play the complete narration without an abrupt cutoff.
""".strip()
